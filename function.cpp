...
//使用的时候所有值需要减一
const int tableIP[64] = {58, 50, 42, 34, 26, 18, 10, 2,
...
const int tableConverseIP[64] = {40, 8, 48, 16, 56, 24, 64, 32,
...
const int tableE[48] = {32, 1, 2, 3, 4, 5,
                        4, 5, 6, 7, 8, 9,
                        8, 9, 10, 11, 12, 13,
                        12, 13, 14, 15, 16, 17,
                        16, 17, 18, 19, 20, 21,
...
const int tableP[32] = {16, 7, 20, 21, 29, 12, 28, 17,
...
bool compare(int a, int b, const unsigned char *backup) {
    ...
    int j = a % 8;
    //a位上的数是1还是0
    int onA = (backup[i] & maskTable[j]) == maskTable[j];
    int k = b / 8;
    int h = b % 8;
    //b位上的数是1还是0
    int onB = (backup[k] & maskTable[h]) == maskTable[h];
    return (onA == onB);
}
